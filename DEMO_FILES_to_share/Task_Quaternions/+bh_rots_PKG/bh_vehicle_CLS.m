...
%                 0,        0,  1; ];
%
%    OBJ = OBJ.rotate_using_gRb(gRb);
%
%    figure();
%       hax = axes();  OBJ.plot_3D(hax);
%--------------------------------------------------------------------------
    ...
    methods
        ...
        %------------------------------------------------------------------
        function OBJ = rotate_using_gRb(OBJ, agRb)
            
            % get the BODY frame co-ordinates
            [X,Y,Z] = OBJ.get_B_XYZ();
            ...
            % update the NEW G-frame co-ordinates              
           OBJ = OBJ.set_G_XYZ( new_XYZ(1,:)', new_XYZ(2,:)', new_XYZ(3,:)');
            
           % update the gRb property used in plotting the arrows
           OBJ.gRb = agRb;
        end
        %------------------------------------------------------------------
        function [OBJ, hax] = rotate_and_animate(OBJ, arot_OBJ, hax)
            
            import bh_rots_PKG.bh_rot_active_B2G_CLS
    ...
    methods (Access = protected)
        ...
        function OBJ = PROT_set_G_XYZ(OBJ, Xg, Yg, Zg)
            ...
            tf_test_a =(length(Xg)==length(Yg)) && (length(Xg)==length(Zg));
        ...
        function hax = PROT_plot_3D(OBJ, hax)
             ...
             C(1,1,:) = [0,1,0];
             hp(1) = patch(  X,Y, Z, C, 'FaceAlpha', OBJ.FaceAlpha, 'EdgeAlpha',OBJ.EdgeAlpha); 
             ...
             C(1,1,:) = [1,0,0];
             hp(2) = patch(  X,Y, Z, C, 'FaceAlpha', OBJ.FaceAlpha, 'EdgeAlpha',OBJ.EdgeAlpha);
             ...
             C(1,1,:) = [0,0,1];
             hp(3) = patch(  X,Y, Z, C, 'FaceAlpha', OBJ.FaceAlpha, 'EdgeAlpha',OBJ.EdgeAlpha);
             ...
             C(1,1,:) = [1,1,0];
             hp(4) = patch(  X,Y, Z, C, 'FaceAlpha', OBJ.FaceAlpha, 'EdgeAlpha',OBJ.EdgeAlpha); 
             ...
             C(1,1,:) = [0,1,1];
             hp(5) = patch(  X,Y, Z, C, 'FaceAlpha', OBJ.FaceAlpha, 'EdgeAlpha',OBJ.EdgeAlpha);
             ...
             mArrow3([0 0 0], arrow_yB, 'color', 'green', 'stemWidth', 0.05);
             mArrow3([0 0 0], arrow_zB, 'color', 'blue',  'stemWidth', 0.05);
